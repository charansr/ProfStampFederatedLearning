...
def ratio_tensors_same(changing_tensor, reference_tensor, same_ratio: float):
    if changing_tensor.shape != reference_tensor.shape:
        raise ValueError("diff size")
...
def controllable_mpaf_attack_nn(state_dict, device, weight_ratio, mp_lambda=3) -> OrderedDict:
    mpaf = copy.deepcopy(state_dict)
    ...
    for key in mpaf.keys():
        ...
        print("diff or not: ",torch.max(mpaf[key]-tmp))
        mpaf[key].copy_(tmp)
    return mpaf
